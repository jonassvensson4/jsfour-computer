if ( !jsloaded.includes('medicalrecords') ) {
    let _page = '.medicalrecords-search';
    let _record = false;
    let _data = 'null';

    jsloaded.push('medicalrecords');
    ...
    function fetchUserRecords( type, data, open ) {
        ...
        .then( data => {
            if ( data != 'false' && data.length > 0 ) {
                if ( type === 'user' ) {
                    $( `#${ device }-frame .medicalrecords-record` ).html( ' ' );
                    $( `#${ device }-frame .medicalrecords-record` ).append(`<i class="material-icons mr-back">arrow_back</i><h6>${ rows['@firstname'] } ${ rows['@lastname'] } (${ rows['@dob'] }) Records <i class="material-icons mr-add-shortcut">add</i></h6><ul class="collapsible"></ul>`);
                    ...
                    Object.keys( data ).forEach(( k ) => {
                        ...
                        $( `#${ device }-frame .medicalrecords-record` ).find('.collapsible').append( li );
                    });

                    $('.collapsible').collapsible();
                } else {
                    $( `#${ device }-frame .medicalrecords-result` ).html( ' ' );
                    ...
                    Object.keys( data ).forEach(( k ) => {
                        ...
                        $( `#${ device }-frame .medicalrecords-result` ).append( elem );
                    });
                }

                if ( open ) {
                    $( `#${ device }-frame .medicalrecords-record` ).find('.collapsible').collapsible('open', 0);
                }
            }
        });
    }
    ...
    function searchRecords() {
        ...
        .then( data => {
            if ( data != 'false' && data.length > 0 ) {
                ...
                $( `#${ device }-frame .medicalrecords-search-input` ).addClass('invalid');

                setTimeout(() => {
                    $( `#${ device }-frame .medicalrecords-search-input` ).removeClass('invalid');
                }, 2000);
            }
        });
    }
    ...
    $('.medicalrecords-add-form').submit(() => {
        let inputs = $(`#${ device }-frame .medicalrecords-add-form`).find('input');
        let rows = {};

        rows['@text'] = $(`#${ device }-frame .medicalrecords-add-form`).find('textarea').val();
        rows['@date'] = getDate();
